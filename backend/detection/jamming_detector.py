...
class JammingDetector:
    ...
    def check_jamming(self, signal: Dict[str, Any], signal_history: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if is_jamming:
            evidence = self._collect_jamming_evidence(
                signal, noise_analysis, broadband_interference, jamming_pattern, jamming_confidence
            )
    ...
    def _calculate_baseline_noise(self, frequency: float, signal_history: List[Dict[str, Any]]) -> float:
        """
        ...
        relevant_signals = [
            s for s in signal_history[-50:]  # Last 50 signals
    ...
    def _detect_continuous_jamming(self, signal: Dict[str, Any], signal_history: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        if mean_power > -30 and power_variance < 25:  # -30 dBm threshold, 25 dB² variance threshold
            confidence = min(1.0, (mean_power + 50) / 50)  # Scale confidence based on power
            duration = len(recent_signals) * 0.1  # Assume 100ms per signal
            return {'confidence': confidence, 'duration': duration}
        
        return {'confidence': 0.0, 'duration': 0.0}
    
    def _detect_pulse_jamming(self, signal: Dict[str, Any], signal_history: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        if len(powers) > 1:
            power_threshold = np.mean(powers) + 1.5 * np.std(powers)  # 1.5 sigma above mean
        else:
        ...
        if len(peaks) >= 3:
            ...
            if len(peak_intervals) > 1:
                ...
                if mean_interval > 0 and interval_variance / (mean_interval ** 2) < 0.1:  # Coefficient of variation < 0.1
                    pulse_rate = 1.0 / mean_interval if mean_interval > 0 else 0
                    confidence = min(1.0, len(peaks) / 10.0)  # Scale by number of detected pulses
                    return {'confidence': confidence, 'pulse_rate': pulse_rate}
        
        return {'confidence': 0.0, 'pulse_rate': 0.0}
    
    def _detect_sweep_jamming(self, signal: Dict[str, Any], signal_history: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        if len(freq_changes) > 3:
            ...
            positive_changes = sum(1 for fc in freq_changes if fc > 1000)  # > 1 kHz change
            negative_changes = sum(1 for fc in freq_changes if fc < -1000)  # < -1 kHz change
    ...
    def _detect_spot_jamming(self, signal: Dict[str, Any], signal_history: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if len(spectrum) > 10:
            ...
            if peak_to_average > 10.0 and max_power > 2.0:  # 10:1 ratio threshold and minimum power
                ...
                peak_frequency = frequency + (peak_index - len(spectrum)//2) * freq_resolution
    ...
    def _calculate_jamming_confidence(self, noise_analysis: Dict[str, Any], 
        ...
        if noise_analysis['elevation_significant']:
            noise_factor = min(1.0, abs(noise_analysis['noise_elevation']) / (2 * self.noise_threshold))
        ...
        if confidence_factors:
            ...
            if len(confidence_factors) >= 2 or (len(confidence_factors) == 1 and final_confidence > 0.8):
    ...
    def _collect_jamming_evidence(self, signal: Dict[str, Any], noise_analysis: Dict[str, Any],
                                broadband_interference: bool, jamming_pattern: Dict[str, Any],
                                jamming_confidence: float) -> JammingEvidence:
        """

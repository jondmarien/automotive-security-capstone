...
class ReplayAttackDetector:
        ...
        for historical_signal in recent_signals:
            ...
            if similarity > self.similarity_threshold:
                ...
                if self._has_replay_timing_characteristics(signal, historical_signal):
                    evidence = self._collect_replay_evidence(signal, historical_signal, similarity)
    ...
    def _get_recent_signals(self, signal_history: List[Dict[str, Any]], time_window: int) -> List[Dict[str, Any]]:
    ...
    def _calculate_signal_similarity(self, signal1: Dict[str, Any], signal2: Dict[str, Any]) -> float:
        ...
        spectrum_similarity = self._compare_power_spectra(
            features1.get('power_spectrum', []),
            features2.get('power_spectrum', [])
        )
        
        # Compare burst timing (30% weight)
        timing_similarity = self._compare_burst_timing(
            features1.get('burst_timing', []),
            features2.get('burst_timing', [])
        )
    ...
    def _has_replay_timing_characteristics(self, signal: Dict[str, Any], historical_signal: Dict[str, Any]) -> bool:
        """
    ...
    def _detect_timing_anomalies(self, signal: Dict[str, Any], historical_signal: Dict[str, Any]) -> bool:
        """
        ...
        if timing1 and timing2 and len(timing1) == len(timing2):
            ...
            timing_precision = np.std(timing_diff) if len(timing_diff) > 1 else abs(timing_diff[0]) if len(timing_diff) == 1 else 0
    ...
    def _collect_replay_evidence(self, signal: Dict[str, Any], historical_signal: Dict[str, Any], similarity: float) -> ReplayEvidence:
        """
    ...
    def _analyze_timing_anomaly(self, signal: Dict[str, Any], historical_signal: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if timing1 and timing2 and len(timing1) == len(timing2):
            ...
            timing_diff = np.array(timing1) - np.array(timing2)
            anomaly_details['timing_precision'] = float(np.std(timing_diff)) if len(timing_diff) > 1 else float(abs(timing_diff[0])) if len(timing_diff) == 1 else 0.0

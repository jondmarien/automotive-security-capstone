...
class TestAutomotiveSignalAnalyzer:
    ...
    def generate_test_signal(
        self, signal_type="key_fob", duration=0.1, noise_level=0.1
    ):
        ...
        if signal_type == "key_fob":
            ...
            for i in range(4):
                ...
                if np.any(burst_mask):  # Only if there are samples in this burst
                    ...
                    freq_pattern = np.where(
                        np.random.random(np.sum(burst_mask)) > 0.5, f1, f2
                    )

                    burst_signal = burst_amplitude * np.exp(
                        1j * 2 * np.pi * freq_pattern * t[burst_mask]
                    )
            ...
            signal = np.zeros(len(t), dtype=complex)
            signal[burst_mask] = burst_amplitude * np.exp(
                1j * 2 * np.pi * freq_deviation * t[burst_mask]
            )

        elif signal_type == "noise":
            # Generate white noise
            signal = (
                np.random.normal(0, 1, len(t)) + 1j * np.random.normal(0, 1, len(t))
            ) * noise_level
        ...
        # Add noise
        noise = np.random.normal(0, noise_level, len(t)) + 1j * np.random.normal(
            0, noise_level, len(t)
        )
    ...
    def test_power_spectrum_computation(self):
        ...
        assert (
            np.max(power_spectrum) > np.mean(power_spectrum) + 10
        )  # At least 10dB above mean
    ...
    def test_burst_timing_detection(self):
        ...
        assert all(
            burst_timing[i] <= burst_timing[i + 1] for i in range(len(burst_timing) - 1)
        )
    ...
    def test_confidence_score_calculation(self):
        ...
        key_fob_confidence = self.analyzer.calculate_confidence_score(
            "key_fob", features
        )
        ...
        unknown_confidence = self.analyzer.calculate_confidence_score(
            "unknown", features
        )

...
def test_event_scrolling():
    ...
    for idx in test_indices:
        ...
        print(
            f"   Selected event: {selected_event['type']} at {selected_event['timestamp']}"
        )
        print(f"   Threat level: {selected_event['threat']}")
        print(
            f"   Technical evidence available: {'technical_evidence' in selected_event}"
        )
    ...
    print("   • Each position should show different signal analysis data")

    print("\\n💡 To test interactively:")
    print("   1. Run: uv run python cli_dashboard.py --mock")
    print("   2. Use Up/Down arrow keys to scroll through events")
    print("   3. Watch the 'Signal Analysis' and 'Technical Evidence' panels")
    print("   4. They should update to show data for the highlighted event")

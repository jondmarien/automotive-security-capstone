...
class TestSignalHistoryBuffer:
    ...
    def test_get_recent_signals(self):
        ...
        assert (
            recent_found or len(recent_signals) > 0
        )  # Either found the specific signal or got some recent signals
    ...
    def test_find_similar_signals(self):
        ...
        similar_signals = self.buffer.find_similar_signals(
            reference_signal, similarity_threshold=0.3
        )
...
class TestThreatDetectionEngine:
    ...
    def test_analyze_replay_attack(self):
        ...
        with patch.object(
            self.threat_detector.replay_detector, "check_replay"
        ) as mock_check:
...
class TestJammingDetector:
    ...
    def test_normal_signal_not_jamming(self):
        ...
        normal_signal = self.create_test_signal(
            bandwidth=50000, rssi=-70, burst_count=3
        )
...
class TestEnhancedSignalProcessingBridge:
    ...
    def setup_method(self):
        ...
        self.bridge = EnhancedSignalProcessingBridge(
            self.mock_rtl_server, rtl_tcp_host="localhost", rtl_tcp_port=1234
        )

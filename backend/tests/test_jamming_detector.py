...
class TestJammingDetector:
    ...
    def setup_method(self):
        ...
        self.detector = JammingDetector(noise_threshold=10.0, interference_threshold=0.8)
    ...
    def test_no_jamming_detected(self):
        ...
        result = self.detector.check_jamming(copy.deepcopy(self.base_signal), signal_history)
    ...
    def test_noise_floor_analysis(self):
        ...
        noise_analysis = self.detector._analyze_noise_floor(jamming_signal, signal_history)
    ...
    def test_broadband_interference_detection(self):
        ...
        broadband_signal['features']['power_spectrum'] = (np.ones(1024) * 2.0).tolist()  # High power flat spectrum
    ...
    def test_continuous_jamming_detection(self):
        ...
        result = self.detector._detect_continuous_jamming(current_signal, signal_history)
    ...
    def test_spot_jamming_false_positive_prevention(self):
        ...
        assert result['confidence'] == 0.0  # Should not detect due to low absolute power
        ...
        assert result['confidence'] == 0.0  # Should not detect due to low peak-to-average ratio
    
    def test_jamming_confidence_calculation(self):
        """Test overall jamming confidence calculation."""
        noise_analysis = {
            'elevation_significant': True,
            'noise_elevation': 20.0
        }
    ...
    def test_high_confidence_jamming_detection(self):
        ...
        jamming_signal['features']['power_spectrum'] = np.ones(1024).tolist()  # Broadband
    ...
    def test_baseline_noise_calculation(self):
        ...
        for i in range(20):
            signal = self.base_signal.copy()
            signal['frequency'] = target_freq + np.random.normal(0, 500000)  # Within 500 kHz
            signal['features']['noise_floor'] = -80.0 + np.random.normal(0, 2)
            signal_history.append(signal)
    ...
    def test_malformed_signal_data(self):
        ...
        result = self.detector.check_jamming(malformed_signal, [copy.deepcopy(self.base_signal)])
    ...
    def test_pattern_identification(self):
        ...
        pattern_result = self.detector._identify_jamming_pattern(self.base_signal, signal_history)
    ...
    def test_threat_level_classification(self):
        ...
        with patch.object(self.detector, '_calculate_jamming_confidence', return_value=0.95):
        ...
        with patch.object(self.detector, '_calculate_jamming_confidence', return_value=0.85):
            result = self.detector.check_jamming(self.base_signal, [])
            assert result['threat_level'] == ThreatLevel.SUSPICIOUS
        
        # Test low confidence (benign)
        with patch.object(self.detector, '_calculate_jamming_confidence', return_value=0.5):
            result = self.detector.check_jamming(self.base_signal, [])
            assert result['threat_level'] == ThreatLevel.BENIGN

...
def test_dashboard_performance_integration():
    ...
    monitor.update_system_health(
        rtl_sdr_connected=True, pico_w_connected=True, memory_usage_mb=95.2
    )
    ...
    print(
        f"   Status: Source: MOCK DATA | {dashboard_summary} | Time: 23:28:45 | Press ? for help"
    )
    ...
    if detailed_report["analysis"]["bottlenecks"]:
        print(
            f"   Bottlenecks: {', '.join(detailed_report['analysis']['bottlenecks'])}"
        )
    else:

...
from utils.simple_performance_monitor import SimplePerformanceMonitor, get_performance_monitor, reset_performance_monitor
...
class PerformanceMonitoringDemo:
    ...
    def __init__(self, duration_seconds: int = 30, num_events: int = 50):
        ...
        print(f"🚗 Automotive Security Performance Monitoring Demo")
    ...
    async def _run_monitoring_simulation(self):
        ...
        with Live(self._create_display(layout, 0, 0), refresh_per_second=4, console=self.console) as live:
            
            while time.time() - start_time < self.duration_seconds and events_generated < self.num_events:
    ...
    def _create_display(self, layout: Layout, elapsed_time: float, events_generated: int) -> Layout:
        """Create the live display layout."""
        
        # Header
        header_text = Text("🚗 Automotive Security Performance Monitor", style="bold cyan")
        header_text.append(f" | Elapsed: {elapsed_time:.1f}s | Events: {events_generated}", style="dim")
        layout["header"].update(Panel(header_text, border_style="cyan"))
        
        # Metrics table
        metrics_table = self._create_metrics_table()
        layout["metrics"].update(Panel(metrics_table, title="Performance Metrics", border_style="green"))
        
        # Recent events
        events_display = self._create_events_display()
        layout["events"].update(Panel(events_display, title="System Status", border_style="yellow"))
    ...
    def _create_metrics_table(self) -> Table:
        ...
        # Performance metrics
        latency_status = "✓" if metrics['average_latency_ms'] < 100 else "⚠️" if metrics['average_latency_ms'] < 200 else "❌"
        table.add_row("Avg Latency", f"{metrics['average_latency_ms']:.1f}ms", latency_status)
    ...
    def _create_events_display(self) -> Table:
        ...
        # System health
        rtl_status = "🟢 Connected" if metrics['rtl_sdr_status'] == 'Connected' else "🔴 Disconnected"
        table.add_row("RTL-SDR V4", rtl_status, "433.92 MHz")
        
        pico_status = "🟢 Connected" if metrics['pico_w_status'] == 'Connected' else "🔴 Disconnected"
        table.add_row("Pico W", pico_status, "Alert System")
        
        # Memory usage
        memory_status = "🟢 Normal" if metrics['memory_usage_mb'] < 200 else "🟡 High" if metrics['memory_usage_mb'] < 500 else "🔴 Critical"
        table.add_row("Memory Usage", memory_status, f"{metrics['memory_usage_mb']:.1f} MB")
    ...
    async def _display_final_results(self):
        ...
        # Summary table
        summary_table = Table(title="Performance Summary", show_header=True, header_style="bold green")
        summary_table.add_column("Metric", style="cyan")
        summary_table.add_column("Value", style="green")
        
        summary_table.add_row("Total Signals Processed", str(summary['signals_processed']))
        summary_table.add_row("Total Events Generated", str(summary['events_generated']))
        summary_table.add_row("Threats Detected", str(summary['threats_detected']))
        summary_table.add_row("Average Latency", f"{summary['average_latency_ms']:.1f}ms")
...
async def main():
    ...
    parser.add_argument("--events", type=int, default=50, help="Number of events to simulate")

...
async def test_rtl_sdr():
    ...
    if detected:
        ...
        if caps:
            ...
            print(
                f"   Automotive Support: {'✅ YES' if caps.supports_automotive_frequencies() else '❌ NO'}"
            )
...
async def test_hardware_manager():
    ...
    if success:
        ...
        print(
            f"   Hardware Ready: {'✅ YES' if manager.is_hardware_ready() else '❌ NO'}"
        )
...
async def run_comprehensive_test():
    ...
    print("-" * 20)
    print(
        f"RTL-SDR Hardware: {'✅ PASS' if results['rtl_sdr'] else '❌ FAIL (Mock Available)'}"
    )
    print(f"Pico W Manager: {'✅ PASS' if results['pico_manager'] else '❌ FAIL'}")
    print(
        f"Hardware Manager: {'✅ PASS' if results['hardware_manager'] else '❌ FAIL'}"
    )

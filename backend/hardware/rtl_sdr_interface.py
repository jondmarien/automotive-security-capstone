...
class RTLSDRInterface:
    ...
    def __init__(self, rtl_sdr_bin_path: Optional[str] = None):
        ...
        logger.info(
            f"RTL-SDR interface initialized with binary path: {rtl_sdr_bin_path}"
        )
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                if "No supported devices found" in result.stderr:
                    logger.error(
                        "No RTL-SDR devices found - check USB connection and drivers"
                    )
            ...
            if self.capabilities:
                ...
                logger.info(
                    f"Frequency range: {self.capabilities.frequency_range[0] / 1e6:.1f} - {self.capabilities.frequency_range[1] / 1e6:.1f} MHz"
                )
    ...
    def configure_for_automotive(self, frequency_band: str = "key_fob_eu") -> bool:
        ...
        if not (
            self.capabilities.frequency_range[0]
            <= frequency
            <= self.capabilities.frequency_range[1]
        ):
        ...
        logger.info("RTL-SDR configured for automotive monitoring:")
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        if self.status in [
            RTLSDRStatus.DETECTED,
            RTLSDRStatus.CONFIGURED,
            RTLSDRStatus.CONNECTED,
        ]:
            try:

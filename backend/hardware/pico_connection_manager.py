...
class PicoConnectionManager:
    ...
    def __init__(self, server_host: str = '0.0.0.0', server_port: int = 8888):
        ...
        logger.info(f"Pico connection manager initialized on {server_host}:{server_port}")
    ...
    async def start_server(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Starting Pico connection server on {self.server_host}:{self.server_port}")
            
            self.server = await asyncio.start_server(
                self._handle_pico_connection,
                self.server_host,
                self.server_port
            )
            
            logger.info(f"Pico connection server listening on {self.server_host}:{self.server_port}")
            return True
    ...
    async def _handle_pico_connection(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        """
                ...
                while '\n' in buffer:
                    ...
                    if line:
                        try:
                            ...
                            logger.warning(f"Invalid JSON from {device.device_id}: {line} - {e}")
    ...
    async def _process_device_message(self, device: PicoDevice, message: Dict[str, Any]) -> None:
        """
        ...
        if message_type == 'heartbeat':
            ...
            logger.debug(f"Unknown message type from {device.device_id}: {message_type}")
    ...
    async def broadcast_to_devices(self, data: Dict[str, Any]) -> int:
        ...
        if self.mock_mode:
            ...
            logger.debug(f"Mock broadcast to {len(self.mock_devices)} devices: {data.get('type', 'unknown')}")
    ...
    async def _check_device_health(self) -> None:
        ...
        for device_id, device in list(self.connected_devices.items()):
            ...
            if device.last_heartbeat and (current_time - device.last_heartbeat) > self.heartbeat_timeout:
    ...
    def get_connected_devices(self) -> List[str]:
        ...
        return [device_id for device_id, device in self.connected_devices.items() 
                if device.status == PicoStatus.CONNECTED]
    ...
    async def check_health(self) -> Dict[str, Any]:
        ...
        connected_count = len([d for d in self.connected_devices.values() 
    ...
    def get_diagnostic_info(self) -> Dict[str, Any]:
        ...
        devices_to_check = self.mock_devices if self.mock_mode else self.connected_devices

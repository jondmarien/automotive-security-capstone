...
class HardwareRecoverySystem:
    ...
    def __init__(self, rtl_sdr: RTLSDRInterface, pico_manager: PicoConnectionManager):
        ...
        self.component_health: Dict[str, ComponentHealth] = {
            ...
            'pico_connections': ComponentHealth('pico_connections', True, time.time(), 0, 0)
        }
        ...
        # Callbacks
        self.failure_callbacks: List[Callable[[str, FailureType, Dict[str, Any]], None]] = []
        self.recovery_callbacks: List[Callable[[str, RecoveryStatus, Dict[str, Any]], None]] = []
    ...
    async def _check_rtl_sdr_health(self, current_time: float) -> None:
        ...
        if not is_healthy:
            await self._handle_component_failure(
                ...
                {'rtl_health': rtl_health}
            )
        else:
            # Reset failure count on successful check
            if not health.is_healthy:
                logger.info(f"RTL-SDR health restored")
    ...
    async def _check_pico_health(self, current_time: float) -> None:
        ...
        if not is_healthy:
            await self._handle_component_failure(
                component,
                FailureType.DISCONNECTION,
                {'pico_health': pico_health}
            )
        else:
            # Reset failure count on successful check
            if not health.is_healthy:
                logger.info(f"Pico connections health restored")
                health.last_recovery_time = current_time
            
            health.is_healthy = True
            health.failure_count = 0
    
    async def _handle_component_failure(self, component: str, failure_type: FailureType, details: Dict[str, Any]) -> None:
        """
        ...
        if health.is_healthy:
            logger.warning(f"Component failure detected: {component} ({failure_type.value})")
        ...
        if (health.failure_count >= self.failure_threshold and 
            component not in self.active_recoveries):
            
            logger.info(f"Failure threshold reached for {component}, triggering recovery")
            await self._schedule_recovery(component, failure_type)
    
    async def _schedule_recovery(self, component: str, failure_type: FailureType) -> None:
        """
        ...
        if health.recovery_attempts >= self.max_recovery_attempts:
            logger.warning(f"Max recovery attempts reached for {component}, activating fallback")
        ...
        logger.info(f"Scheduling recovery for {component} in {delay:.1f} seconds (attempt {health.recovery_attempts + 1})")
    ...
    async def _execute_recovery(self, component: str, failure_type: FailureType, delay: float) -> None:
        """
        ...
        try:
            ...
            logger.info(f"Attempting recovery for {component} (attempt {health.recovery_attempts})")
            ...
            if success:
                ...
                logger.info(f"Recovery successful for {component} in {attempt.recovery_time:.1f}s")
                ...
                await self._notify_recovery_callbacks(component, RecoveryStatus.SUCCESS, {
                ...
                attempt.error_message = f"Recovery attempt {health.recovery_attempts} failed"
                
                logger.warning(f"Recovery failed for {component} (attempt {health.recovery_attempts})")
                ...
                # Notify recovery callbacks
                await self._notify_recovery_callbacks(component, RecoveryStatus.FAILED, {
                    'attempt_number': attempt.attempt_number,
    ...
    async def _recover_pico_connections(self) -> bool:
        ...
        try:
            ...
            if connected > 0:
                logger.info(f"Pico connections recovery successful ({connected} devices)")
                return True
            else:
                logger.warning("Pico connections recovery failed - no devices connected")
                return False
    ...
    async def _activate_fallback(self, component: str) -> None:
        ...
        try:
            ...
            # Notify recovery callbacks
            await self._notify_recovery_callbacks(component, RecoveryStatus.FALLBACK_ACTIVATED, {
                'fallback_mode': 'mock'
            })
    ...
    async def _notify_failure_callbacks(self, component: str, failure_type: FailureType, details: Dict[str, Any]) -> None:
    ...
    async def _notify_recovery_callbacks(self, component: str, status: RecoveryStatus, details: Dict[str, Any]) -> None:
    ...
    def add_failure_callback(self, callback: Callable[[str, FailureType, Dict[str, Any]], None]) -> None:
        """Add callback for hardware failure events."""
        self.failure_callbacks.append(callback)
    
    def add_recovery_callback(self, callback: Callable[[str, RecoveryStatus, Dict[str, Any]], None]) -> None:
    ...
    def get_recovery_history(self, component: Optional[str] = None) -> List[RecoveryAttempt]:
        """
        ...
        """
        if component:
            return [attempt for attempt in self.recovery_history if attempt.component == component]
        ...
        return {
            ...
            'components': {
                name: {
                    ...
                    'last_failure_age': (current_time - health.last_failure_time) if health.last_failure_time else None,
                    'last_recovery_age': (current_time - health.last_recovery_time) if health.last_recovery_time else None
                }

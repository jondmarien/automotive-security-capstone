...
class HardwareManager:
    ...
    def __init__(self, enable_auto_recovery: bool = True):
        ...
        self.hardware_failure_callbacks: List[Callable[[str, Dict[str, Any]], None]] = []
    ...
    async def initialize(self, mock_mode: bool = False) -> bool:
        ...
        if rtl_success and pico_success:
            ...
            logger.warning("Some hardware components failed to initialize - system degraded")
        else:
        ...
        return self.system_status in [SystemStatus.READY, SystemStatus.DEGRADED, SystemStatus.MOCK_MODE]
    ...
    async def _perform_health_check(self) -> None:
        ...
        if not rtl_health['hardware_detected'] and self.rtl_sdr.get_status() != RTLSDRStatus.MOCK_MODE:
            recovery_needed.append('rtl_sdr')
        
        if pico_health['connected_devices'] == 0 and not self.pico_manager.is_mock_mode():
    ...
    async def _attempt_recovery(self, failed_components: List[str]) -> None:
        ...
        for component in failed_components:
            ...
            if recovery_success:
                ...
                await self._notify_hardware_failure(component, {'attempts': attempts + 1})
    ...
    async def _update_system_status(self, rtl_health: Dict[str, Any], pico_health: Dict[str, Any]) -> None:
        ...
        if new_status != previous_status:
            ...
            logger.info(f"System status changed: {previous_status.value} -> {new_status.value}")
    ...
    def add_status_change_callback(self, callback: Callable[[SystemStatus], None]) -> None:
        """Add callback for system status changes."""
        self.status_change_callbacks.append(callback)
    
    def add_hardware_failure_callback(self, callback: Callable[[str, Dict[str, Any]], None]) -> None:
    ...
    async def _notify_hardware_failure(self, component: str, details: Dict[str, Any]) -> None:
    ...
    def _on_hardware_failure(self, component: str, failure_type: FailureType, details: Dict[str, Any]) -> None:
        """Handle hardware failure notifications from recovery system."""
        logger.warning(f"Hardware failure detected by recovery system: {component} ({failure_type.value})")
        
        # Notify registered callbacks
        asyncio.create_task(self._notify_hardware_failure(component, {
            'failure_type': failure_type.value,
            'details': details
        }))
    
    def _on_hardware_recovery(self, component: str, status: RecoveryStatus, details: Dict[str, Any]) -> None:

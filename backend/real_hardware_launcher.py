...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('real_hardware_system.log')
    ]
)
...
class RealHardwareSystem:
    ...
    async def _report_hardware_status(self):
        ...
        if rtl_sdr.get_capabilities():
            ...
            logger.info(f"   Frequency Range: {caps.frequency_range[0]/1e6:.1f} - {caps.frequency_range[1]/1e6:.1f} MHz")
        
        pico_health = await pico_manager.check_health()
        logger.info(f"   Pico W Status: {pico_health['connected_devices']} devices connected")
    ...
    async def _wait_for_pico_connections(self):
        ...
        for i in range(30):
            ...
            if i % 5 == 0:  # Log every 5 seconds
                logger.info(f"Still waiting for Pico W connections... ({30-i}s remaining)")
            ...
            if pico_manager.is_mock_mode():
                ...
                logger.warning("⚠️  No real Pico W devices connected - system will continue without alerts")
    ...
    async def _print_system_summary(self):
        ...
        logger.info(f"📱 Pico W: {health.pico_connections}/{health.total_pico_devices} connected")
    ...
    async def _report_system_health(self):
        ...
        try:
            health = self.hardware_manager.get_health_status()
            recovery_health = self.hardware_manager.get_recovery_system().get_system_health_summary()
            
            logger.info(f"💓 Health Check - Status: {health.system_status.value}, "
            ...
            if recovery_health['active_recoveries']:
                logger.info(f"🔄 Active recoveries: {recovery_health['active_recoveries']}")
            
            if recovery_health['recent_failures'] > 0:
                logger.warning(f"⚠️  Recent failures: {recovery_health['recent_failures']}")

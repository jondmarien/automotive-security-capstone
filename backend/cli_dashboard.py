...
from utils.signal_constants import TIMESTAMP_FORMAT
...
def calculate_threat_statistics(events):
    ...
    for event in events:
        ...
        is_multi_modal = (
            "Multi-modal attack detected" in details
            or "multi_modal" in details.lower()
            or nfc_correlated
            or event.get("threat_category") == "multi_modal_attack"
        )
...
def render_landing_screen_content(args):
    ...
    logo_panel = Panel(
        Align.center(
            Group(
                ...
                Text(
                    "4th Year Cybersecurity Capstone Project (2025)", style="italic dim"
                ),
            )
        ),
    ...
    config_info.add_row(
        "🔍 Signal Analysis:", "✅ Enabled" if args.detailed else "❌ Disabled"
    )
    config_info.add_row(
        "📱 NFC Correlation:", "✅ Enabled" if args.nfc else "❌ Disabled"
    )

    if args.event != -1:
        config_info.add_row(
            "👁️  Event Focus:", f"Event #{args.event} (manual selection)"
        )
    else:
    ...
    config_panel = Panel(
        config_info,
        title="[bold]Active Configuration[/bold]",
        border_style="green",
        padding=(1, 2),
    )
    ...
    help_info.add_column(
        justify="center", style="dim white"
    )  # Descriptions are centered
    ...
    help_panel = Panel(
        help_info,
        title="[bold]Keyboard Controls[/bold]",
        border_style="yellow",
        padding=(1, 2),
    )
...
def display_enhanced_startup_screen(args, startup_delay=2.5):
    ...
    console.print(
        Align.center("4th Year Cybersecurity Capstone Project (2025)"),
        style="italic dim",
    )
    console.print("\n")
    ...
    console.print(
        Align.center(
            "[bold white]Press any key to launch dashboard, or 'q' to quit...[/bold white]"
        )
    )
    console.print(
        Align.center(
            "[dim](Arrow keys will navigate events once dashboard loads)[/dim]"
        )
    )
...
def render_dashboard(
    ...
    layout["body"].split_column(
        ...
        Layout(
            name="bottom_panels", ratio=3
        ),  # Increased bottom section for analysis and evidence
    )

    # Split bottom panels into signal metrics and evidence panel
    layout["bottom_panels"].split_row(
        Layout(name="signal_metrics", ratio=1), Layout(name="evidence_panel", ratio=1)
    )
    ...
    threat_counter_text.append(
        f"Events: {threat_stats['total_events']} ", style="bright_white"
    )
    threat_counter_text.append("| ", style="dim white")
    threat_counter_text.append(
        f"Threats: {threat_stats['threat_count']} ", style="bold red"
    )
    threat_counter_text.append("[", style="dim white")
    threat_counter_text.append(
        f"Suspicious: {threat_stats['suspicious_count']}", style="orange1"
    )
    threat_counter_text.append(" | ", style="dim white")
    threat_counter_text.append(
        f"Malicious: {threat_stats['malicious_count']}", style="red"
    )
    threat_counter_text.append(" | ", style="dim white")
    threat_counter_text.append(
        f"Critical: {threat_stats['critical_count']}", style="bold red"
    )
    if threat_stats["multi_modal_count"] > 0:
        threat_counter_text.append(" | ", style="dim white")
        threat_counter_text.append(
            f"Multi-Modal: {threat_stats['multi_modal_count']}", style="bold magenta"
        )
    threat_counter_text.append("]", style="dim white")

    # Combine gradient header with threat counter
    header_content = Group(create_gradient_header(), Align.center(threat_counter_text))
    ...
    table.add_column(
        "Details", style="bright_white", width=30
    )  # Fixed width for details to prevent affecting other columns
    ...
    signal_viz = (
        render_signal_metrics(selected_event, console_width=width)
        if selected_event
        else Align.center(Text("No signal data available"))
    )
    evidence_panel = (
        render_evidence_panel(selected_event, console_width=width)
        if selected_event
        else Align.center(Text("No evidence data available"))
    )
    ...
    for i, event in enumerate(page_events):
        ...
        if selected_event_idx == -1:
            ...
            target_index = (
                len(events) + selected_event_idx
                if selected_event_idx < 0
                else selected_event_idx
            )
    ...
    bottom_panel_height = max(
        8, min(12, height // 4)
    )  # Between 8-12 lines based on screen height

    layout["signal_metrics"].update(
        Panel(
            signal_viz,
    ...
    if performance_summary and performance_summary != "Monitoring...":
        enhanced_status_text = (
            f"{status_text} | {event_count_text} | {performance_summary}"
        )
    else:
...
def render_evidence_panel(event, console_width=None):
    ...
    is_security_event = (
        event.get("type", "") in security_types
        or event.get("threat", "") in ["Malicious", "Critical", "Suspicious"]
        or event.get("nfc_correlated", False)
        or "NFC" in event.get("type", "")
    )

    if not is_security_event:
        return Align.center(
            Text("No security evidence for this event", style="dim italic")
        )
    ...
    if (
        not evidence
        and not technical_evidence
        and not event.get("nfc_correlated", False)
        and "NFC" not in event.get("type", "")
    ):
    ...
    tree = Tree(
        f"🛡️  [bold yellow]{event.get('type', 'Security Event')} Analysis[/]",
    ...
    if evidence.get("temporal_analysis"):
        ...
        temp_branch.add(
            f"{temp.get('detection_count', 0)} detections in {temp.get('time_window_seconds', 0)}s"
        )
        temp_branch.add(f"Burst interval: {temp.get('burst_interval_ms', 0)}ms")

    if evidence.get("burst_pattern"):
        tree.add(
            f":chart_increasing: [bold]Pattern:[/] [blue]{evidence['burst_pattern']}[/]"
        )

    if (
        "NFC" in event.get("type", "")
        or event.get("nfc_correlated", False)
        or event.get("nfc_tag_id")
    ):
    ...
    if evidence.get("peak_frequencies"):
        ...
        peak_str = ", ".join(
            [f"{p / 1e6:.3f}MHz" if p > 1e6 else f"{p / 1e3:.1f}kHz" for p in peaks]
        )
    ...
    if technical_evidence:
        ...
        for item in technical_evidence:
            ...
            icon = evidence_icons.get(
                item_type.lower().replace(" ", "_"), evidence_icons["default"]
            )
            ...
            for key, value in details.items():
                ...
                branch.add(
                    f"[green]{formatted_key}:[/green] [bright_white]{value}[/bright_white]"
                )
...
def render_signal_metrics(event, console_width=None):
    ...
    if not is_technical:
        return Align.center(
            Text("Event does not contain RF signal metrics", style="dim")
        )
    ...
    if isinstance(rssi, (int, float)):
        ...
        rssi_percent = min(
            100, max(0, (rssi + 100) * 1.25)
        )  # Scale from -100..-20 to 0..100%
        rssi_color = (
            "green" if rssi_percent > 70 else "yellow" if rssi_percent > 30 else "red"
        )
    ...
    if isinstance(snr, (int, float)):
        ...
        snr_color = (
            "green" if snr_percent > 70 else "yellow" if snr_percent > 30 else "red"
        )
    ...
    if burst_count > 0:
        burst_blocks = "#" * min(
            burst_count, 20
        )  # Limit to 20 blocks max, using ASCII hash instead of block
...
def get_signal_summary(event):
    ...
    if (
        "modulation_type" not in event
        and "frequency" not in event
        and "rssi" not in event
    ):
        return "-"
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Automotive Security Enhanced CLI Dashboard"
    )
    parser.add_argument(
        "--source", choices=["api", "tcp"], help="Event source: api or tcp"
    )
    parser.add_argument(
        "--api-url",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="localhost",
        help="TCP event stream host (default: localhost)",
    )
    parser.add_argument(
        "--tcp-port",
        type=int,
        default=8888,
        help="TCP event stream port (default: 8888)",
    )
    parser.add_argument(
        "--mock",
        action="store_true",
        help="Enable mock mode to simulate detection events for testing and demo purposes.",
    )
    parser.add_argument(
        "--synthetic",
        action="store_true",
        help="Enable synthetic signal generation for advanced testing (requires --mock)",
    )
    parser.add_argument(
        "--detailed",
        action="store_true",
        default=True,
        help="Enable detailed signal analysis (default: True)",
    )
    parser.add_argument(
        "--nfc",
        action="store_true",
        default=True,
        help="Enable NFC correlation detection (default: True)",
    )
    parser.add_argument(
        "--event",
        type=int,
        default=-1,
        help="Select a specific event to view (negative index counts from latest, default: -1 shows latest)",
    )
    ...
    if args.synthetic and not args.mock:
        console.print(
            "[bold red]Error:[/] --synthetic flag requires --mock flag. Exiting.",
            style="bold red",
        )
        return
    ...
    if args.mock:
        ...
        if args.synthetic:
            ...
            dashboard_logger.info(
                "Using synthetic signal generator with advanced testing features"
            )
        else:
        ...
        dashboard_logger.info(
            f"Using TCP event source: {args.tcp_host}:{args.tcp_port}"
        )
    ...
    def handle_up(event):
        ...
        if dashboard_logger:
            log_dashboard_action(
                dashboard_logger,
                "navigation",
                f"Up arrow - selected event index: {selected_event_idx}",
            )
    ...
    def handle_down(event):
        ...
        if dashboard_logger:
            log_dashboard_action(
                dashboard_logger,
                "navigation",
                f"Down arrow - selected event index: {selected_event_idx}",
            )
        ...
        if events:
            ...
            if dashboard_logger:
                log_dashboard_action(
                    dashboard_logger,
                    "navigation",
                    f"Home key - selected event index: {selected_event_idx}",
                )
    ...
    def handle_end(event):
        ...
        if dashboard_logger:
            log_dashboard_action(
                dashboard_logger,
                "navigation",
                "End key - jumped to latest event (auto-follow enabled)",
            )
    ...
    def handle_quit(event):
        ...
        if dashboard_logger:
            log_dashboard_action(
                dashboard_logger, "quit", "User initiated dashboard shutdown"
            )
    ...
    def handle_instant_exit(event):
        ...
        if dashboard_logger:
            log_dashboard_action(
                dashboard_logger,
                "instant_exit",
                "User initiated instant shutdown (Ctrl+C)",
            )
    ...
    def handle_previous_page(event):
        ...
        if dashboard_logger:
            log_dashboard_action(
                dashboard_logger,
                "navigation",
                f"Left arrow - moved to page {current_page + 1}",
            )
    ...
    def handle_next_page(event):
        ...
        if dashboard_logger:
            log_dashboard_action(
                dashboard_logger,
                "navigation",
                f"Right arrow - moved to page {current_page + 1}",
            )
    ...
    def handle_help(event):
        ...
        if dashboard_logger:
            log_dashboard_action(
                dashboard_logger,
                "help_toggle",
                f"Help display {'enabled' if show_help else 'disabled'}",
            )
    ...
    def handle_main_menu(event):
        ...
        if dashboard_logger:
            log_dashboard_action(
                dashboard_logger,
                "landing_screen_toggle",
                f"Landing screen display {'enabled' if show_landing else 'disabled'}",
            )
        ...
        try:
            ...
            with Live(
                ...
                async def event_fetcher():
                    ...
                    async for event in event_gen:
                        ...
                        if (
                            event.get("type") == "signal_detection"
                            and "detections" in event
                        ):
                            for detection in event["detections"]:
                                # Add timestamp if not present
                                if "timestamp" not in detection:
                                    detection["timestamp"] = event.get(
                                        "timestamp",
                        ...
                        if (
                            event.get("error")
                            and "API connection error" in event["error"]
                        ):
                            api_error_count += 1
                            if api_error_count >= MAX_API_ERRORS:
                                console.print(
                                    f"[bold red]Unable to connect to API after {MAX_API_ERRORS} attempts. Exiting dashboard.[/bold red]"
                                )
                                return
                        else:
                            api_error_count = 0
                ...
                async def renderer():
                    ...
                    FULL_REFRESH_RATE = (
                        1.0  # Slower refresh for evidence trees (1 second)
                    )
                    STATUS_REFRESH_RATE = (
                        0.2  # Faster refresh for status updates (200ms)
                    )
                    PERFORMANCE_LOG_RATE = (
                        30.0  # Log performance metrics every 30 seconds
                    )
                    ...
                    while running:
                        ...
                        if events:
                            ...
                            selected_event_idx = max(
                                -len(events), min(-1, selected_event_idx)
                            )
                            ...
                            if first_absolute_event_requested:
                                ...
                                selected_event["details"] = (
                                    f"[First event (0/{total_events_processed})] {selected_event.get('details', '')}"
                                )
                                abs_idx = 0
                            else:
                            ...
                            event_num = (
                                abs_idx + 1
                            )  # Simple 1-based indexing from current events
                            ...
                            event_status = (
                                f"[cyan1]Events: {current_position}/{len(events)}"
                            )
                            if selected_event_idx == -1:
                            ...
                            time_status = (
                                f"[spring_green3]Time: {display_time}[/spring_green3]"
                            )

                            # Get accurate threat counter using same logic as header
                            threat_stats = calculate_threat_statistics(events)
                            threats_status = (
                                f"[bold red]Threats: {threat_stats['threat_count']}[/bold red]"
                                if threat_stats["threat_count"] > 0
                                else "[green]No Active Threats[/green]"
                            )

                            # Comprehensive status with all elements and colors
                            source_text = (
                                "Source: SYNTHETIC SIGNALS (advanced testing)"
                                if args.synthetic
                                else "Source: MOCK DATA (demo/testing mode)"
                            )
                            status_parts = [
                                source_text,
                                event_status,
                        ...
                        if (
                            needs_full_refresh
                            or current_time - last_full_refresh >= FULL_REFRESH_RATE
                        ):
                            ...
                            if show_landing:
                                ...
                                live.update(
                                    Panel(
                                        landing_content,
                                        title="[bold blue]AutoSec Monitor - Landing Screen[/bold blue]",
                                        border_style="cyan",
                                    )
                                )
                            else:
                                ...
                                live.update(
                                    render_dashboard(
                                        ...
                                        show_help=show_help,
                                        current_page=current_page,
                                        force_refresh=needs_full_refresh,
                                    )
                                )
                            ...
                            if show_landing:
                                ...
                                status_only = render_dashboard(
                                    events,
                                    None if selected_event else None,
                                    full_status,
                                    console,
                                    selected_event_idx,
                                    status_only=True,
            ...
            if dashboard_logger:
                dashboard_logger.error(
                    f"Unexpected error in dashboard: {e}", exc_info=True
                )
        finally:
            if dashboard_logger:
        ...
        if exit_requested:
            try:
                if instant_exit_requested:
                    ...
                    exit_manager = ExitDialogManager(
                        console, events, dashboard_logger, args.synthetic
                    )
                    exit_manager.show_final_goodbye(
                        exported_data=False
                    )  # No export on instant exit
                    break
                else:
...
def enhance_event_with_signal_details(event):
    ...
    if "NFC" in enhanced.get("type", "") or enhanced.get("nfc_correlated", False):
        ...
        if (
            enhanced.get("threat", "") in ["Malicious", "Suspicious"]
    ...
    if enhanced.get("nfc_correlated", False):
        ...
        enhanced["details"] = (
            f"[bold red]!!! [/] {enhanced.get('details', 'Multi-modal attack detected')}"
        )
...
def flatten_detection(event, detection):
    ...
    flat = {
        ...
        "Signal Quality": f"[green]{detection.get('signal_quality', '-'):.1f}[/green]"
        if detection.get("signal_quality", "-") > 0.5
        else f"[red]{detection.get('signal_quality', '-'):.1f}[/red]",
        "Signal Strength": f"[green]{detection.get('signal_strength', '-'):.1f}[/green]"
        if detection.get("signal_strength", "-") > 0.5
        else f"[red]{detection.get('signal_strength', '-'):.1f}[/red]",

...
class PicoDeployer:
    ...
    def find_pico_port(self):
        ...
        for port in ports:
            ...
            if any(
                identifier in str(port).lower()
                for identifier in ["pico", "rp2040", "micropython"]
            ):
                pico_ports.append(port.device)
                logger.info(
                    f"Found potential Pico W on {port.device}: {port.description}"
                )
    ...
    def connect(self):
        ...
        try:
            ...
            if ">>>" in response or "MicroPython" in response:
                ...
                logger.warning(
                    f"Connected but no MicroPython prompt found. Response: {response}"
                )
...
def main():
    ...
    try:
        ...
        if config_path.exists():
            with open(config_path, "r") as f:
                ...
                if "YOUR_WIFI_NETWORK_NAME" in config_content:
                    ...
                    logger.warning(
                        "Please update WIFI_SSID, WIFI_PASSWORD, and TCP_HOST in config.py"
                    )
                    logger.warning(
                        "Deployment will continue, but Pico W won't connect without proper config."
                    )
        ...
        logger.info(
            f"Deployment complete: {success_count}/{len(files_to_deploy)} files uploaded"
        )
        ...
        logger.info(
            "The Pico W should now attempt to connect to WiFi and the main system."
        )

...
class ExitDialogManager:
    ...
    def _create_session_summary(self) -> Panel:
        ...
        summary_table.add_row("Critical Threats:", str(threat_counts.get('critical', 0)))
        summary_table.add_row("Malicious Events:", str(threat_counts.get('malicious', 0)))
        summary_table.add_row("Suspicious Events:", str(threat_counts.get('suspicious', 0)))
    ...
    def _get_export_preferences(self) -> Dict[str, bool]:
        ...
        try:
            ...
            export_options['events'] = Confirm.ask(
                "📊 Export event history (JSON format)?",
                default=True
            )
            
            # CSV report export
            export_options['csv_report'] = Confirm.ask(
                "📈 Export summary report (CSV format)?",
                default=True
            )
            ...
            export_options['metadata'] = Confirm.ask(
                "⚙️  Export session metadata and configuration?",
                default=False
            )
            
        except KeyboardInterrupt:
            self.console.print("\n[bold yellow]Using default export options...[/bold yellow]")
            export_options = {'events': True, 'csv_report': True, 'logs': False, 'metadata': False}
    ...
    def export_session_data(self, export_options: Dict[str, bool]) -> bool:
        ...
        if not any(export_options.values()):
            self.console.print("[yellow]No data export requested. Exiting cleanly.[/yellow]")
            return True
        ...
        with Progress(
            ...
            if export_options.get('events', False):
                ...
                try:
                    ...
                    self.console.print(f"✅ Events exported to: [green]{events_file}[/green]")
                    self.console.print()  # Add spacing
            ...
            if export_options.get('csv_report', False):
                ...
                try:
                    ...
                    self.console.print(f"❌ Failed to export CSV report: [red]{e}[/red]")
                    export_success = False
            ...
            if export_options.get('metadata', False):
                ...
                try:
                    ...
                    progress.update(task, completed=1)
                    self.console.print(f"✅ Metadata exported to: [green]{metadata_file}[/green]")
                    self.console.print()  # Add spacing
                except Exception as e:
    ...
    def _export_csv_report(self, csv_file: Path):
        ...
        with open(csv_file, 'w', newline='', encoding='utf-8') as f:
            ...
            # Write header
            writer.writerow([
                'Timestamp', 'Event Type', 'Threat Level', 'Source', 
                'Frequency', 'RSSI', 'Description', 'Evidence Count'
            ])
    ...
    def _copy_log_files(self, timestamp: str):
        ...
        for log_file in log_files:
            if os.path.exists(log_file):
                dest_file = self.export_dir / f"{log_file.replace('.log', '')}_{timestamp}.log"
    ...
    def _export_session_metadata(self, metadata_file: Path):
        ...
        metadata = {
            ...
            'session_summary': {
                ...
                'threat_counts': self._calculate_threat_counts(),
                'session_duration': self._calculate_session_duration(),
                'first_event_time': self.events[0].get('timestamp', '') if self.events else '',
                'last_event_time': self.events[-1].get('timestamp', '') if self.events else ''
            },
            'export_info': {
    ...
    def show_export_summary(self, export_success: bool):
        ...
        if export_success:
            ...
            summary_text = Text()
            summary_text.append("✅ Data export completed successfully!\n\n", style="green")
            summary_text.append("All requested files have been saved to:\n", style="green")
            ...
            summary_text.append("Check the console output above for details.\n", style="yellow")
    ...
    def show_final_goodbye(self, exported_data=False):
        ...
        lines = []
        lines.append(Align.center(Text("Thank you for using the Automotive Security Dashboard!", style="bold green")))
        lines.append(Text())  # Empty line for spacing
        
        # Only show export message if data was actually exported
        if exported_data:
            lines.append(Align.center(Text("Session data has been preserved and exported.", style="green")))
        else:
            lines.append(Align.center(Text("Session data remains in memory only.", style="yellow")))

...
from .automotive_signal_analyzer import AutomotiveSignalAnalyzer, DetectedSignal
...
class ThreatDetectionEngine:
    ...
    def analyze_threat(self, detected_signal: DetectedSignal) -> Dict[str, Any]:
        ...
        brute_force_result = self.brute_force_detector.check_brute_force(
            detected_signal
        )
    ...
    def _generate_threat_event(
        self, detected_signal: DetectedSignal, threat_indicators: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
...
class ReplayAttackDetector:
    ...
    def check_replay(self, detected_signal: DetectedSignal) -> Dict[str, Any]:
        ...
        similar_signals = self.signal_history.find_similar_signals(
            signal_data, self.similarity_threshold, self.time_window
        )
        ...
        if similar_signals:
            ...
            for similar_signal in similar_signals:
                ...
                if self._has_replay_timing_characteristics(
                    detected_signal, similar_signal, time_diff
                ):
                    return {
                        "is_replay": True,
                        "confidence": self._calculate_replay_confidence(
                            detected_signal, similar_signal
                        ),
                        "evidence": {
                            "original_timestamp": similar_signal.timestamp,
                            "replay_timestamp": detected_signal.timestamp,
                            "time_difference": time_diff,
                            "signal_similarity": self._calculate_similarity(
                                detected_signal, similar_signal
                            ),
                            "timing_anomaly": self._analyze_timing_anomaly(
                                detected_signal, similar_signal
                            ),
                        },
                    }
    ...
    def _has_replay_timing_characteristics(
        self, current_signal: DetectedSignal, historical_signal, time_diff: float
    ) -> bool:
    ...
    def _calculate_replay_confidence(
    ...
    def _calculate_similarity(
        ...
        return self.signal_history._calculate_signal_similarity(
            current_data, historical_signal.signal_data
        )

    def _calculate_timing_match(
        self, current_signal: DetectedSignal, historical_signal
    ) -> float:
    ...
    def _analyze_timing_anomaly(
        ...
        return {
            "burst_count_match": len(current_signal.features.burst_timing)
            == len(historical_signal.features.get("burst_timing", [])),
            "inter_burst_similarity": self._calculate_timing_match(
                current_signal, historical_signal
            ),
            "temporal_separation": current_signal.timestamp
            - historical_signal.timestamp,
        }
...
class EnhancedSignalProcessingBridge:
    ...
    def __init__(self, rtl_server_manager, rtl_tcp_host="localhost", rtl_tcp_port=1234):
        ...
        self.signal_analyzer = AutomotiveSignalAnalyzer(
            sample_rate=rtl_server_manager.sample_rate
        )
        ...
        logger.info(
            f"EnhancedSignalProcessingBridge initialized: host={rtl_tcp_host}, port={rtl_tcp_port}"
        )
            ...
            try:
                ...
                while self.processing_active:
                    ...
                    if len(self.rtl_server.connected_picos) > 0:
                        processed_events = await self.process_samples(
    ...
    async def process_samples(
        self, raw_data: bytes, sample_count: int
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for detected_signal in detected_signals:
                ...
                self.performance_monitor.record_event_generated(
                    threat_analysis.get("event_type", "unknown")
                )

                # Log significant detections
                if threat_analysis["threat_level"] > 0.5:
                    log(
                        f"[THREAT] {threat_analysis['event_type']} detected: "
    ...
    def _create_event_from_analysis(
        self, threat_analysis: Dict[str, Any], sample_count: int
    ) -> Dict[str, Any]:

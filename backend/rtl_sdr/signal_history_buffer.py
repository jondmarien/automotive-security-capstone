...
class SignalHistoryBuffer:
    ...
    def __init__(self, max_size: int = 1000, time_window: float = 300.0):
        ...
        logger.info(f"SignalHistoryBuffer initialized: max_size={max_size}, time_window={time_window}s")
    ...
    def add_signal(self, signal_data: Dict[str, Any]) -> str:
        ...
        with self._lock:
            ...
            logger.debug(f"Added signal {signal_id} to buffer (buffer size: {len(self._buffer)})")
            return signal_id
    ...
    def get_recent_signals(self, time_window: Optional[float] = None) -> List[StoredSignal]:
        """
        ...
        with self._lock:
            recent_signals = [
                signal for signal in self._buffer
                if signal.timestamp >= cutoff_time
            ]
            
        logger.debug(f"Retrieved {len(recent_signals)} signals from last {time_window}s")
        return recent_signals
    
    def get_signals_by_type(self, signal_type: str, time_window: Optional[float] = None) -> List[StoredSignal]:
        """
        ...
        logger.debug(f"Retrieved {len(filtered_signals)} signals of type '{signal_type}'")
        return filtered_signals
    
    def find_similar_signals(self, target_signal: Dict[str, Any], 
                           similarity_threshold: float = 0.8,
                           time_window: Optional[float] = None) -> List[StoredSignal]:
        """
        ...
        for stored_signal in recent_signals:
            similarity = self._calculate_signal_similarity(target_signal, stored_signal.signal_data)
            
            if similarity >= similarity_threshold:
                similar_signals.append(stored_signal)
                logger.debug(f"Found similar signal {stored_signal.signal_id} with similarity {similarity:.3f}")
        ...
        if features1 and features2:
            ...
            if 'power_spectrum' in features1 and 'power_spectrum' in features2:
                power_sim = self._compare_power_spectra(
                    features1['power_spectrum'], 
                    features2['power_spectrum']
                )
            ...
            if 'burst_timing' in features1 and 'burst_timing' in features2:
                timing_sim = self._compare_burst_timing(
                    features1['burst_timing'],
                    features2['burst_timing']
                )
                similarity_score += timing_sim * 0.3
                comparison_count += 1
            
            # Compare frequency deviation
            if 'frequency_deviation' in features1 and 'frequency_deviation' in features2:
                freq_sim = self._compare_frequency_deviation(
                    features1['frequency_deviation'],
                    features2['frequency_deviation']
                )
            ...
            if 'signal_bandwidth' in features1 and 'signal_bandwidth' in features2:
                bw_sim = self._compare_bandwidth(
                    features1['signal_bandwidth'],
                    features2['signal_bandwidth']
                )
    ...
    def _compare_power_spectra(self, spectrum1: List[float], spectrum2: List[float]) -> float:

...
with open(ANALYZE_DIR + "byte_shift_alignment.txt", "w") as out:
    ...
    for shift in range(max_len):
        ...
        invariant_cols = [
            i for i, col in enumerate(columns) if len(set(x for x in col if x)) == 1
        ]
        ...
        out.write(
            "Invariant columns: "
            + (", ".join(str(i) for i in invariant_cols) if invariant_cols else "None")
            + "\n"
        )
...
with open(ANALYZE_DIR + "pairwise_clusters.txt", "w") as out:
    ...
    for leader, members in clusters.items():
        out.write(
            f"Cluster led by Burst {leader + 1}: {', '.join(str(m + 1) for m in members)}\n"
        )
...
with open(ANALYZE_DIR + "bit_level_invariance.txt", "w") as out:
    ...
    out.write(
        "Burst | "
        + " ".join(f"{i:>2}" for i in range(min(64, max_bits)))
        + (" ..." if max_bits > 64 else "")
        + "\n"
    )
    out.write(
        "-" * (7 + 3 * min(64, max_bits)) + ("..." if max_bits > 64 else "") + "\n"
    )

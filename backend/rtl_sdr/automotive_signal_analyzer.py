...
class SignalFeatures:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'power_spectrum': self.power_spectrum.tolist() if isinstance(self.power_spectrum, np.ndarray) else self.power_spectrum,
...
class AutomotiveSignalAnalyzer:
    ...
    def __init__(self, sample_rate: float = 2048000):
        ...
        logger.info(f"AutomotiveSignalAnalyzer initialized with sample_rate={sample_rate}")
    ...
    def _detect_burst_timing(self, complex_samples: np.ndarray) -> List[float]:
        ...
        smoothed_power = np.convolve(instantaneous_power, 
        ...
        if std_power < mean_power * 0.1:
            ...
            threshold = mean_power + 2 * std_power  # Reduced from 3 to 2 for better sensitivity
        ...
        gap_threshold = max(window_size, int(self.sample_rate * 0.005))  # 5ms gap minimum
    ...
    def _measure_bandwidth(self, complex_samples: np.ndarray, power_spectrum: np.ndarray) -> float:
    ...
    def _find_peak_frequencies(self, power_spectrum: np.ndarray) -> List[float]:
        ...
        peaks, properties = scipy_signal.find_peaks(power_spectrum, 
                                                   height=np.max(power_spectrum) - 10,
                                                   distance=10)
    ...
    def _calculate_inter_burst_intervals(self, burst_timing: List[float]) -> List[float]:
    ...
    def detect_automotive_patterns(self, features: SignalFeatures) -> List[DetectedSignal]:
        """
    ...
    def _detect_key_fob_pattern(self, features: SignalFeatures) -> Optional[DetectedSignal]:
        ...
        # Check burst count
        if (self.detection_thresholds['min_burst_count'] <= 
            features.burst_count <= self.detection_thresholds['max_burst_count']):
            confidence += 0.2
    ...
    def _detect_tpms_pattern(self, features: SignalFeatures) -> Optional[DetectedSignal]:
    ...
    def calculate_confidence_score(self, signal_type: str, features: SignalFeatures) -> float:
        """

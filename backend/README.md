...
- `cli_dashboard.py`: **ENHANCED** - Rich-powered CLI dashboard with professional startup/exit experience, enhanced event navigation (20 events per page), advanced synthetic event generation, and comprehensive session data export capabilities; supports `--mock` for demo/testing, `--synthetic` for advanced testing scenarios, and `--event` for specific event selection
...
### CLI Dashboard Enhancements (LATEST)

- `utils/exit_dialog.py`: **NEW** - Professional exit experience with Rich dialogs, session data export options (events, logs, reports), and clean terminal restoration
- `cli_dashboard_detection_adapter.py`: **ENHANCED** - Advanced synthetic event generation with realistic attack scenarios including replay attacks, jamming patterns, brute force sequences, signal cloning, relay attacks, and critical exploits
- Enhanced startup experience with ASCII art timing, menu loading screens, and centered text display
- Improved event navigation with 20 events per page and fixed jitter issues
- Comprehensive session data export functionality with proper file handling
- Professional confirmation dialogs for exit scenarios (quit 'q' or Ctrl+C)
- Enhanced synthetic event generation with uncapped event limits and proper critical event categorization
...
1. **Install RTL-SDR drivers/tools** (see [docs/poc_migration_plan.md](docs/plans/migration_old/poc_migration_plan.md))
...
3. **Quick Start for Demo/Testing (RECOMMENDED)**
   ...
   ```sh
      # Basic dashboard (polls API)
      uv run python cli_dashboard.py --source api --api-url http://localhost:8000/events

      # TCP stream mode (connects to rtl_tcp_server.py)
      uv run python cli_dashboard.py --source tcp --tcp-host 127.0.0.1 --tcp-port 8888

      # Mock/demo mode (no hardware required) - ENHANCED with professional experience
      uv run python cli_dashboard.py --mock

      # Advanced synthetic event testing with realistic attack scenarios
      uv run python cli_dashboard.py --mock --synthetic

      # Professional Exit Experience:
      # - Press 'q' or Ctrl+C during operation for graceful exit
      # - Choose to export session data (events, logs, reports)
      # - Clean terminal restoration and professional presentation
   ```

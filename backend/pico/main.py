...
class AutomotiveSecurityPico:
    ...
    def init_nfc(self):
        ...
        try:
            ...
            reset_pin = (
                Pin(config.NFC_SDA_PIN, Pin.OUT)
                if hasattr(config, "NFC_RESET_PIN")
                else None
            )
    ...
    async def connect_to_server(self, server_ip=None, server_port=None):
        ...
        while attempt < max_attempts:
            ...
            try:
                print(
                    "Connecting to server {}:{} (attempt {})".format(
    ...
    async def process_signal_detection(self, detection_data):
        ...
        for detection in detections:
            ...
            if getattr(config, "DEBUG_MODE", False):
                print(
                    "Detection {}: {} (threat: {})".format(
                        self.detection_count, signal_type, threat_level
                    )
                )
            ...
            if isinstance(threat_level, (float, int)) and threat_level > 0.7:
                is_high_threat = True
            elif isinstance(threat_level, str) and threat_level.lower() in (
                "high",
                "critical",
                "suspicious",
                "malicious",
    ...
    async def activate_nfc_correlation(self, detections):
        ...
        print(
            "[NFC CORRELATION] Activating NFC scanning for {} high-threat events".format(
                len(detections)
            )
        )
        ...
        self.active_rf_threat = (
            detections[0] if detections else None
        )  # Store first high-threat detection
    ...
    async def generate_security_alert(self, detection, context):
        ...
        if not isinstance(threat_level, (float, int, str)):
            print(
                "[DEBUG] Unexpected threat_level type in alert:",
                type(threat_level),
                threat_level,
            )
        ...
        if not isinstance(power_db, (float, int)):
            print(
                "[DEBUG] Unexpected power_db type in alert:", type(power_db), power_db
            )
    ...
    def get_recommended_action(self, detection):
        ...
        if signal_type == "potential_jamming":
            ...
            signal_type == "key_fob_transmission"
            and isinstance(threat_level, (float, int))
    ...
    def log_alert_locally(self, alert):
        ...
        print(
            "LOCAL LOG: {} - {}".format(
                alert["alert_id"], alert["detection_details"]["event_type"]
            )
        )
    ...
    async def handle_nfc_detection(self, uid):
        ...
        if self.nfc_correlation_mode and self.active_rf_threat:
            print(
                "[CORRELATION] NFC detection during RF threat: {}".format(
                    self.active_rf_threat.get("event_type", "unknown")
                )
            )

            # Generate enhanced correlated security event
            correlated_event = self.create_correlated_security_event(
                self.active_rf_threat, nfc_data
            )
    ...
    def create_correlated_security_event(self, rf_threat, nfc_detection):
        ...
        escalated_threat_level = min(
            1.0, base_threat_level + 0.3
        )  # Escalate by 0.3 (capped at 1.0)
        ...
        recommended_action = self.get_correlated_recommended_action(
        ...
        correlated_event = {
            ...
            "event_id": "correlated_{}_{}".format(
                int(time.time()), self.detection_count
            ),
            "timestamp": time.time(),
            "rf_threat": rf_threat,
            ...
            "confidence_score": self.calculate_correlation_confidence(
                rf_threat, nfc_detection
            ),
        }
    ...
    def collect_technical_evidence(self, rf_threat, nfc_detection):
        ...
        evidence = {
            ...
            "correlation_evidence": {
                "time_delta_seconds": time.time()
                - rf_threat.get("timestamp", time.time()),
                "proximity_indicators": [
                    "rf_nfc_temporal_correlation",
                    "multi_modal_attack_pattern",
                ],
            },
        }
    ...
    def calculate_correlation_confidence(self, rf_threat, nfc_detection):
        ...
        temporal_confidence = (
            0.8  # High confidence since NFC was detected during correlation mode
        )
        ...
        threat_confidence = min(
            1.0, rf_threat_level + 0.2
        )  # Boost confidence based on RF threat
